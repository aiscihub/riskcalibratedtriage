...
def _apply_core_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Historical exclusions already used in the project
    exclude = (
    ...
    # Detect protein-pocket groups with all endpoint info missing
    endpoint_cols = ["label_unstable", "rmsd_20ns", "ligand_drift"]
    bad_groups = (
        df.groupby(["protein", "pocket"])[endpoint_cols]
        .apply(lambda g: g.isna().all().all())
    )
    bad_groups = bad_groups[bad_groups].index

    if len(bad_groups) > 0:
        bad_set = set(bad_groups)
        df = df[~df[["protein", "pocket"]].apply(tuple, axis=1).isin(bad_set)].copy()

    # Now remove rows with missing labels
...
def _threshold_by_youden_j(y_true, y_prob):
    """Choose threshold maximizing Youden's J = TPR - FPR on TRAIN data."""
...
def _resolve_rmsd_heuristic_column(df: pd.DataFrame, preferred: str | None = None) -> str:
    """Resolve a column for the simple RMSD-magnitude heuristic baseline.

    Preferred order tries explicit RMSD-magnitude columns first, then falls back to
    the structural RMSD-derived signal already present in the compact fingerprint.
    """
    candidates = []
    if preferred:
        candidates.append(preferred)
    candidates.extend([
        'pocket_rmsd_mean', 'pocket_rmsd_avg', 'pocket_rmsd', 'rmsd_mean',
        'rmsd_avg', 'rmsd', 'pocket_rmsd_max', 'rmsd_max', 'slope'
    ])
    for col in candidates:
        if col in df.columns:
            return col
    raise ValueError(
        f"Could not find an RMSD heuristic column. Tried: {candidates}. Available columns include: {list(df.columns)[:25]}..."
    )


def evaluate_priority1_decision_baselines(
        features_path: Path,
        timescales: list,
        ...
        long_ns: float = 20.0,
        heuristic_column: str | None = None,
        lr_c: float = 3.0,
):
    """Evaluate the revision-critical baselines for the decision policy.

    Baselines returned:
      - LR_FPR: existing method (train-fold FPR-calibrated threshold)
      - LR_0.5: same LR score, fixed 0.5 threshold
      - LR_Youden: same LR score, train-fold Youden-J threshold
      - RMSD_heuristic: single-feature heuristic using RMSD magnitude only

    Output shape matches the fold_ops-oriented reporting already used elsewhere.
    """
    from sklearn.model_selection import GroupKFold
    ...
    for t in timescales:
        ...
        df_raw = _apply_core_filters(df_raw)
        df_t = df_raw[df_raw['time_ns'] == t].copy()
        heuristic_col = _resolve_rmsd_heuristic_column(df_t, preferred=heuristic_column)
        df_t = df_t.dropna(subset=feature_cols + ['label_unstable', 'protein', heuristic_col])

        if group_by == 'ligand_pocket':
            if 'ligand_name' in df_t.columns:
                groups_h = (df_t['protein'].astype(str) + '|' +
                            df_t['ligand_name'].astype(str) + '|' +
                            df_t['pocket'].astype(str)).values
            else:
                groups_h = (df_t['protein'].astype(str) + '|' + df_t['pocket'].astype(str)).values
        elif group_by == 'pocket':
            groups_h = (df_t['protein'].astype(str) + '|' + df_t['pocket'].astype(str)).values
        else:
            groups_h = df_t['protein'].values

        if len(df_t) != len(X):
            raise ValueError(f"Mismatch between LR matrix rows ({len(X)}) and heuristic rows ({len(df_t)}). Make sure feature/NaN filtering is aligned.")

        H = df_t[[heuristic_col]].values.astype(float)
        ...
        by_baseline = {
            'LR_FPR': {'fold_ops': [], 'y_true': [], 'y_score': []},
            'LR_Platt_FPR': {'fold_ops': [], 'y_true': [], 'y_score': []},
            'LR_0.5': {'fold_ops': [], 'y_true': [], 'y_score': []},
            'LR_Youden': {'fold_ops': [], 'y_true': [], 'y_score': []},
            'RMSD_heuristic': {'fold_ops': [], 'y_true': [], 'y_score': []},
        }
        ...
        for fold_id, (train_idx, test_idx) in enumerate(cv.split(X, y, groups)):
            ...
            # Same LR score used for all LR decision rules
            X_train_s = scaler_lr.fit_transform(X_train)
            X_test_s = scaler_lr.transform(X_test)
            lr = LogisticRegression(max_iter=1000, class_weight='balanced', C=lr_c, random_state=42)
            lr.fit(X_train_s, y_train)
            p_train = lr.predict_proba(X_train_s)[:, 1]
            p_test = lr.predict_proba(X_test_s)[:, 1]

            # Calibration-aware baseline: Platt-scaled LR, calibrated on TRAIN only
            lr_cal = CalibratedClassifierCV(
                estimator=LogisticRegression(max_iter=1000, class_weight='balanced', C=lr_c, random_state=42),
                method='sigmoid',
                cv=3,
            )
            lr_cal.fit(X_train_s, y_train)
            pcal_train = lr_cal.predict_proba(X_train_s)[:, 1]
            pcal_test = lr_cal.predict_proba(X_test_s)[:, 1]

            # Heuristic baseline: single RMSD-magnitude feature trained as 1D LR
            H_train_s = scaler_h.fit_transform(H_train)
            H_test_s = scaler_h.transform(H_test)
            h_lr = LogisticRegression(max_iter=1000, class_weight='balanced', C=lr_c, random_state=42)
            h_lr.fit(H_train_s, y_train)
            h_train = h_lr.predict_proba(H_train_s)[:, 1]
            h_test = h_lr.predict_proba(H_test_s)[:, 1]

            by_baseline['LR_FPR']['y_true'].extend(y_test.tolist())
            by_baseline['LR_FPR']['y_score'].extend(p_test.tolist())
            by_baseline['LR_Platt_FPR']['y_true'].extend(y_test.tolist())
            by_baseline['LR_Platt_FPR']['y_score'].extend(pcal_test.tolist())
            by_baseline['LR_0.5']['y_true'].extend(y_test.tolist())
            by_baseline['LR_0.5']['y_score'].extend(p_test.tolist())
            by_baseline['LR_Youden']['y_true'].extend(y_test.tolist())
            by_baseline['LR_Youden']['y_score'].extend(p_test.tolist())
            by_baseline['RMSD_heuristic']['y_true'].extend(y_test.tolist())
            by_baseline['RMSD_heuristic']['y_score'].extend(h_test.tolist())

            # Shared helper for fold metrics
            def append_fold_op(target_store, y_score_train, y_score_test, threshold, baseline_name, fpr_target=None):
                y_hat = (y_score_test >= threshold).astype(int)
                ...
                recall = tp / (tp + fn + 1e-8)
                achieved_fpr = fp / (fp + tn + 1e-8)
                triage_rate = float(y_hat.mean())
                cost_saved = float(triage_rate * (long_ns - t) / long_ns)
                target_store.append({
                    'fold': fold_id,
                    'time_ns': t,
                    'baseline': baseline_name,
                    'fpr_target': None if fpr_target is None else float(fpr_target),
                    'recall': float(recall),
                    'achieved_fpr': float(achieved_fpr),
            ...
            for fpr_target in fpr_targets:
                tau, _, _ = _threshold_at_target_fpr(y_train, p_train, fpr_target)
                if np.isfinite(tau):
                    append_fold_op(by_baseline['LR_FPR']['fold_ops'], p_train, p_test, tau, 'LR_FPR', fpr_target)
                tau_cal, _, _ = _threshold_at_target_fpr(y_train, pcal_train, fpr_target)
                if np.isfinite(tau_cal):
                    append_fold_op(by_baseline['LR_Platt_FPR']['fold_ops'], pcal_train, pcal_test, tau_cal, 'LR_Platt_FPR', fpr_target)

            append_fold_op(by_baseline['LR_0.5']['fold_ops'], p_train, p_test, 0.5, 'LR_0.5', None)
            tau_youden = _threshold_by_youden_j(y_train, p_train)
            if np.isfinite(tau_youden):
                append_fold_op(by_baseline['LR_Youden']['fold_ops'], p_train, p_test, tau_youden, 'LR_Youden', None)

            tau_h = _threshold_by_youden_j(y_train, h_train)
            if np.isfinite(tau_h):
                append_fold_op(by_baseline['RMSD_heuristic']['fold_ops'], h_train, h_test, tau_h, 'RMSD_heuristic', None)

        results[t] = {}
        for baseline_name, store in by_baseline.items():
            y_true_all = np.asarray(store['y_true'])
            y_score_all = np.asarray(store['y_score'])
            if len(np.unique(y_true_all)) > 1:
                ...
                auc = np.nan
                pr_auc = np.nan
            results[t][baseline_name] = {
                'auc': float(auc) if np.isfinite(auc) else np.nan,
                'pr_auc': float(pr_auc) if np.isfinite(pr_auc) else np.nan,
                'y_true': y_true_all,
                'y_score': y_score_all,
                'fold_ops': store['fold_ops'],
                'heuristic_column': heuristic_col if baseline_name == 'RMSD_heuristic' else None,
            }
...
# ============================================================================
# SEQUENTIAL / ADAPTIVE STOPPING BASELINE
# ============================================================================

def _build_group_array_from_meta(meta: pd.DataFrame, group_by: str) -> np.ndarray:
    """Build the same group labels used elsewhere from aligned sample metadata."""
    has_ligand = "ligand_name" in meta.columns
    if group_by == "ligand_pocket":
        if has_ligand:
            return (
                meta["protein"].astype(str) + "|" +
                meta["ligand_name"].astype(str) + "|" +
                meta["pocket"].astype(str)
            ).values
        return (meta["protein"].astype(str) + "|" + meta["pocket"].astype(str)).values
    if group_by == "pocket":
        return (meta["protein"].astype(str) + "|" + meta["pocket"].astype(str)).values
    return meta["protein"].astype(str).values


def load_sequential_horizon_data(
        features_path: Path,
        horizons: tuple = (2, 3, 5),
        group_by: str = "protein",
        feature_cols: list | None = None,
):
    """Load a horizon-aligned dataset for sequential stopping.

    Only complexes with complete feature vectors at every requested horizon are
    retained. This prevents sample mismatch across 2/3/5 ns decisions.
    """
    if feature_cols is None:
        feature_cols = FEATURE_COLS
    ...
    if missing:
        if set(missing) == {"energy_std"}:
            log.warning("energy_std column not found, falling back to 4-feature set")
            feature_cols = [c for c in feature_cols if c != "energy_std"]
        else:
            raise ValueError(f"Missing required feature columns: {missing}")

    key_cols = ["protein", "pocket"]
    if "ligand_name" in df.columns:
        key_cols = ["protein", "ligand_name", "pocket"]

    df = df[df["time_ns"].astype(float).isin(horizons)].copy()
    df = df.dropna(subset=feature_cols + ["label_unstable", "protein", "pocket"])

    agg = {c: "mean" for c in feature_cols}
    agg["label_unstable"] = "first"
    df_g = df.groupby(key_cols + ["time_ns"], as_index=False).agg(agg)

    keys_by_h = {}
    for h in horizons:
        sub = df_g[np.isclose(df_g["time_ns"].astype(float), h)].copy()
        keys_by_h[h] = set(map(tuple, sub[key_cols].astype(str).values.tolist()))
    common_keys = set.intersection(*keys_by_h.values()) if keys_by_h else set()
    if len(common_keys) == 0:
        raise ValueError(f"No samples have complete feature rows for horizons={horizons}")

    common_keys = sorted(common_keys)
    meta = pd.DataFrame(common_keys, columns=key_cols)

    X_by_h = {}
    y_by_h = {}
    for h in horizons:
        sub = df_g[np.isclose(df_g["time_ns"].astype(float), h)].copy()
        sub["__key__"] = list(map(tuple, sub[key_cols].astype(str).values.tolist()))
        sub = sub[sub["__key__"].isin(common_keys)].copy()
        sub = sub.set_index("__key__").loc[common_keys]
        X_by_h[h] = sub[feature_cols].values.astype(float)
        y_by_h[h] = sub["label_unstable"].values.astype(int)

    y = y_by_h[horizons[0]]
    for h in horizons[1:]:
        if not np.array_equal(y, y_by_h[h]):
            raise ValueError(f"label_unstable differs across horizons; first mismatch at horizon={h}")

    groups = _build_group_array_from_meta(meta, group_by=group_by)
    return X_by_h, y, groups, meta, feature_cols


def _make_group_cv(cv_mode: str, groups: np.ndarray):
    from sklearn.model_selection import GroupKFold
    if cv_mode == "gkf":
...
def _inner_oof_lr_scores(
    ...
    """Group-aware inner out-of-fold LR probabilities for threshold calibration."""
    ...
    if len(np.unique(y)) < 2 or len(np.unique(groups)) < 2:
        ...
        model = LogisticRegression(max_iter=1000, class_weight="balanced", C=lr_c, random_state=random_state)
        model.fit(Xs, y)
        return model.predict_proba(Xs)[:, 1]

    cv = _make_group_cv(cv_mode, groups)
    for inner_train_idx, inner_val_idx in cv.split(X, y, groups):
        y_inner = y[inner_train_idx]
        if len(np.unique(y_inner)) < 2:
            continue
        scaler = StandardScaler()
        X_inner = scaler.fit_transform(X[inner_train_idx])
        X_val = scaler.transform(X[inner_val_idx])
        model = LogisticRegression(max_iter=1000, class_weight="balanced", C=lr_c, random_state=random_state)
        model.fit(X_inner, y_inner)
        p[inner_val_idx] = model.predict_proba(X_val)[:, 1]

    # Fill skipped validation points with full-training scores for tiny grouped folds.
    ...
    if missing.any():
        ...
        model = LogisticRegression(max_iter=1000, class_weight="balanced", C=lr_c, random_state=random_state)
        model.fit(Xs, y)
        p[missing] = model.predict_proba(Xs[missing])[:, 1]

    return p


def _candidate_thresholds(scores: np.ndarray, max_grid: int = 41) -> np.ndarray:
    """Return a compact candidate threshold grid from scores."""
    ...
    if uniq.size > max_grid:
        qs = np.linspace(0, 1, max_grid)
        uniq = np.unique(np.quantile(uniq, qs))
    return np.unique(np.concatenate([uniq, [np.nextafter(1.0, 2.0)]]))


def _simulate_sequential_policy(
        y_true: np.ndarray,
        scores_by_h: dict,
        thresholds_by_h: dict,
        horizons: tuple,
        long_ns: float = 20.0,
):
    """Apply sequential stopping and return labels, stop times, counts, and metrics."""
    y_true = np.asarray(y_true).astype(int)
    n = len(y_true)
    y_hat = np.zeros(n, dtype=int)
    stop_time = np.full(n, float(long_ns), dtype=float)
    remaining = np.ones(n, dtype=bool)

    n_stop_by_h = {}
    for h in horizons:
        h = float(h)
        scores = np.asarray(scores_by_h[h], dtype=float)
        tau = float(thresholds_by_h[h])
        stop_now = remaining & (scores >= tau)
        y_hat[stop_now] = 1
        stop_time[stop_now] = h
        remaining[stop_now] = False
        n_stop_by_h[h] = int(stop_now.sum())
    ...
    achieved_fpr = fp / (fp + tn + 1e-8)
    triage_rate = float(y_hat.mean())
    saved_fraction = (float(long_ns) - stop_time) / float(long_ns)
    cost_saved = float(np.mean(saved_fraction))

    return {
        "y_hat": y_hat,
        "stop_time": stop_time,
        "n_stop_by_h": n_stop_by_h,
        "tp": tp, "fn": fn, "fp": fp, "tn": tn,
        "recall": float(recall),
        "achieved_fpr": float(achieved_fpr),
        "triage_rate": triage_rate,
        "cost_saved": cost_saved,
        "cost_saved_sum": float(saved_fraction.sum()),
    }


def _select_joint_sequential_thresholds(
        y_train: np.ndarray,
        train_scores_by_h: dict,
        horizons: tuple,
        target_fpr: float,
        long_ns: float = 20.0,
        max_threshold_grid: int = 41,
        objective: str = "cost_then_recall",
):
    """Jointly select thresholds by simulating the full sequential rule.

    This avoids independently calibrating each horizon and OR-combining them,
    which can inflate the sequential FPR.
    """
    horizons = tuple(float(h) for h in horizons)
    grids = {h: _candidate_thresholds(train_scores_by_h[h], max_grid=max_threshold_grid) for h in horizons}
    ...
    h0, h1, h2 = horizons
    for tau0 in grids[h0]:
        for tau1 in grids[h1]:
            for tau2 in grids[h2]:
                thresholds = {h0: float(tau0), h1: float(tau1), h2: float(tau2)}
                m = _simulate_sequential_policy(
                    y_true=y_train,
                    scores_by_h=train_scores_by_h,
                    thresholds_by_h=thresholds,
                    horizons=horizons,
                    long_ns=long_ns,
                )
                if m["achieved_fpr"] > target_fpr + 1e-12:
                    continue

                if objective == "recall_then_cost":
                    key = (m["recall"], m["cost_saved"], -m["achieved_fpr"])
                else:
                    key = (m["cost_saved"], m["recall"], -m["achieved_fpr"])
    ...
    if best_thresholds is None:
        best_thresholds = {float(h): float(np.nextafter(1.0, 2.0)) for h in horizons}
        best_metrics = _simulate_sequential_policy(
            y_true=y_train,
            scores_by_h=train_scores_by_h,
            thresholds_by_h=best_thresholds,
            horizons=horizons,
            long_ns=long_ns,
        )
...
def evaluate_sequential_lr_fpr_baseline(
        features_path: Path,
        horizons: tuple = (2, 3, 5),
        group_by: str = "protein",
        cv_mode: str = "logo",
    ...
    """Evaluate a jointly calibrated sequential LR-FPR stopping baseline.

    At test time, each complex is evaluated at 2 ns, then 3 ns, then 5 ns.
    It stops at the first horizon whose LR score exceeds the jointly calibrated
    threshold; otherwise it continues to 20 ns.
    """
    horizons = tuple(float(h) for h in horizons)
    if len(horizons) != 3:
        raise ValueError("This implementation expects exactly three horizons, e.g. (2, 3, 5).")

    X_by_h, y, groups, meta, used_features = load_sequential_horizon_data(
    ...
    fold_ops = []
    y_true_all = []
    y_pred_all_by_target = {float(a): [] for a in fpr_targets}

    for fold_id, (train_idx, test_idx) in enumerate(cv.split(X_by_h[horizons[0]], y, groups)):
        y_train, y_test = y[train_idx], y[test_idx]
        groups_train = groups[train_idx]

        train_oof_scores_by_h = {}
        test_scores_by_h = {}

        for h in horizons:
            Xh = X_by_h[h]
            X_train_h = Xh[train_idx]
            X_test_h = Xh[test_idx]

            train_oof_scores_by_h[h] = _inner_oof_lr_scores(
                X=X_train_h,
                y=y_train,
                groups=groups_train,
                cv_mode=cv_mode,
            ...
            model = LogisticRegression(max_iter=1000, class_weight="balanced", C=lr_c, random_state=42)
            model.fit(X_train_s, y_train)
            test_scores_by_h[h] = model.predict_proba(X_test_s)[:, 1]

        y_true_all.extend(y_test.tolist())

        for fpr_target in fpr_targets:
            fpr_target = float(fpr_target)
            thresholds, train_metrics = _select_joint_sequential_thresholds(
                y_train=y_train,
                train_scores_by_h=train_oof_scores_by_h,
                horizons=horizons,
                target_fpr=fpr_target,
                long_ns=long_ns,
                max_threshold_grid=max_threshold_grid,
                objective=objective,
            )

            test_metrics = _simulate_sequential_policy(
                y_true=y_test,
                scores_by_h=test_scores_by_h,
                thresholds_by_h=thresholds,
                horizons=horizons,
                long_ns=long_ns,
            )
            y_pred_all_by_target[fpr_target].extend(test_metrics["y_hat"].tolist())

            row = {
                "fold": fold_id,
                "baseline": "Sequential_LR_FPR",
                "horizons": "/".join(str(int(h)) for h in horizons),
                "fpr_target": fpr_target,
                "recall": test_metrics["recall"],
                "achieved_fpr": test_metrics["achieved_fpr"],
                "triage_rate": test_metrics["triage_rate"],
                "cost_saved": test_metrics["cost_saved"],
                "cost_saved_sum": test_metrics["cost_saved_sum"],
                "n_test": int(len(y_test)),
                "tp": test_metrics["tp"], "fn": test_metrics["fn"],
                "fp": test_metrics["fp"], "tn": test_metrics["tn"],
                "train_recall": train_metrics["recall"],
                "train_fpr": train_metrics["achieved_fpr"],
                "train_cost_saved": train_metrics["cost_saved"],
            }
            for h in horizons:
                row[f"tau_{int(h)}ns"] = float(thresholds[h])
                row[f"n_stop_{int(h)}ns"] = int(test_metrics["n_stop_by_h"].get(h, 0))
            fold_ops.append(row)
    ...
    return {
        ...
        "y_pred_by_target": {k: np.asarray(v) for k, v in y_pred_all_by_target.items()},
        "meta": meta,
    }


def export_sequential_baseline_table(seq_results: dict, output_csv: str, output_latex: str | None = None):
    """Export pooled and fold-median summaries for the sequential baseline."""
    ops_all = seq_results.get("fold_ops", [])
    rows = []
    for fpr_target in sorted({float(op["fpr_target"]) for op in ops_all}):
        ops = [op for op in ops_all if np.isclose(float(op["fpr_target"]), fpr_target)]
        if not ops:
            continue
        tp = sum(int(op["tp"]) for op in ops)
        fn = sum(int(op["fn"]) for op in ops)
        fp = sum(int(op["fp"]) for op in ops)
        tn = sum(int(op["tn"]) for op in ops)
        n_test = sum(int(op["n_test"]) for op in ops)
        cost_saved_sum = sum(float(op.get("cost_saved_sum", 0.0)) for op in ops)

        row = {
            "baseline": "Sequential_LR_FPR",
            "horizons_ns": ops[0].get("horizons", "2/3/5"),
            "fpr_target": fpr_target,
            "recall_pooled": tp / (tp + fn + 1e-8),
            "achieved_fpr_pooled": fp / (fp + tn + 1e-8),
            "triage_rate_pooled": (tp + fp) / (n_test + 1e-8),
            "cost_saved_pooled": cost_saved_sum / (n_test + 1e-8),
            "recall_median_iqr": format_median_iqr([100.0 * float(op["recall"]) for op in ops], 1),
            "achieved_fpr_median_iqr": format_median_iqr([100.0 * float(op["achieved_fpr"]) for op in ops], 1),
            "triage_rate_median_iqr": format_median_iqr([100.0 * float(op["triage_rate"]) for op in ops], 1),
            "cost_saved_median_iqr": format_median_iqr([100.0 * float(op["cost_saved"]) for op in ops], 1),
            "n_folds": len(ops),
            "n_test": n_test,
            "tp": tp, "fn": fn, "fp": fp, "tn": tn,
        }
        for h in seq_results.get("horizons", (2, 3, 5)):
            h_int = int(h)
            n_stop = sum(int(op.get(f"n_stop_{h_int}ns", 0)) for op in ops)
            row[f"stop_{h_int}ns_pct"] = 100.0 * n_stop / (n_test + 1e-8)
        rows.append(row)
    ...
    if output_latex:
        with open(output_latex, "w") as f:
            f.write("\\begin{table}[t]\n")
            f.write("\\centering\n")
            f.write("\\caption{Jointly calibrated sequential LR-FPR baseline under cross-protein validation. The policy evaluates 2, 3, and 5 ns windows and stops at the first threshold crossing. Values in parentheses are median [Q1, Q3] across folds.}\n")
            f.write("\\label{tab:sequential_baseline}\n")
            f.write("\\small\n")
            f.write("\\begin{tabular}{lcccc}\n")
            f.write("\\toprule\n")
            f.write("Method & Recall (\\%) & Achieved FPR (\\%) & Terminated (\\%) & Cost saved (\\%)\\\\\n")
            f.write("\\midrule\n")
            for _, r in df.iterrows():
                method = f"Seq. LR-FPR $\\alpha={r['fpr_target']:.2f}$"
                f.write(
                    f"{method} & {100*r['recall_pooled']:.1f} ({r['recall_median_iqr']}) & "
                    f"{100*r['achieved_fpr_pooled']:.1f} ({r['achieved_fpr_median_iqr']}) & "
                    f"{100*r['triage_rate_pooled']:.1f} ({r['triage_rate_median_iqr']}) & "
                    f"{100*r['cost_saved_pooled']:.1f} ({r['cost_saved_median_iqr']})\\\\\n"
                )
            f.write("\\bottomrule\n")
            f.write("\\end{tabular}\n")
            f.write("\\end{table}\n")
...
def export_fixed_vs_sequential_table(
        fixed_results: dict,
        seq_results: dict,
        output_csv: str,
        output_latex: str | None = None,
        fixed_operating_points: tuple = ((5, 0.20), (5, 0.10)),
):
    """Export a compact table comparing fixed-horizon LR-FPR vs sequential LR-FPR."""
    rows = []

    for t_ns, fpr_target in fixed_operating_points:
        if t_ns not in fixed_results or "LR_FPR" not in fixed_results[t_ns]:
            continue
        ops = [op for op in fixed_results[t_ns]["LR_FPR"].get("fold_ops", [])
               if np.isclose(float(op.get("fpr_target", -1)), float(fpr_target))]
        if not ops:
            continue
        tp = sum(int(op["tp"]) for op in ops)
        fn = sum(int(op["fn"]) for op in ops)
        ...
        tn = sum(int(op["tn"]) for op in ops)
        n_test = sum(int(op["n_test"]) for op in ops)
        cost_sum = sum(float(op["cost_saved"]) * int(op["n_test"]) for op in ops)
        rows.append({
            "method": f"Fixed {int(t_ns)} ns LR_FPR",
            "fpr_target": float(fpr_target),
            "recall": tp / (tp + fn + 1e-8),
            "achieved_fpr": fp / (fp + tn + 1e-8),
            "triage_rate": (tp + fp) / (n_test + 1e-8),
            "cost_saved": cost_sum / (n_test + 1e-8),
            "n_folds": len(ops),
            "n_test": n_test,
            "tp": tp, "fn": fn, "fp": fp, "tn": tn,
        })
    ...
    for fpr_target in sorted({float(op["fpr_target"]) for op in ops_all}):
        ...
        cost_saved_sum = sum(float(op.get("cost_saved_sum", 0.0)) for op in ops)
        rows.append({
            "method": "Sequential 2/3/5 ns LR_FPR",
            "fpr_target": fpr_target,
            "recall": tp / (tp + fn + 1e-8),
            "achieved_fpr": fp / (fp + tn + 1e-8),
            "triage_rate": (tp + fp) / (n_test + 1e-8),
            "cost_saved": cost_saved_sum / (n_test + 1e-8),
            "n_folds": len(ops),
            "n_test": n_test,
            "tp": tp, "fn": fn, "fp": fp, "tn": tn,
        })
    ...
    if output_latex:
        with open(output_latex, "w") as f:
            f.write("\\begin{table}[t]\n")
            f.write("\\centering\n")
            f.write("\\caption{Fixed-horizon LR-FPR compared with a jointly calibrated sequential stopping baseline under cross-protein validation. Thresholds are selected on training data only and applied unchanged to held-out proteins.}\n")
            f.write("\\label{tab:fixed_vs_sequential}\n")
            f.write("\\small\n")
            f.write("\\begin{tabular}{lcccc}\n")
            f.write("\\toprule\n")
            f.write("Method & Recall & Achieved FPR & Terminated & Cost saved\\\\\n")
            ...
            for _, r in df.iterrows():
                f.write(
                    f"{r['method']} ($\\alpha={r['fpr_target']:.2f}$) & "
                    f"{100*r['recall']:.1f} & {100*r['achieved_fpr']:.1f} & "
                    f"{100*r['triage_rate']:.1f} & {100*r['cost_saved']:.1f}\\\\\n"
                )
            f.write("\\bottomrule\n")
            f.write("\\end{tabular}\n")
            f.write("\\end{table}\n")
    ...
    return df
def export_baseline_calibration_metrics(
        baseline_results: dict,
        output_csv: str,
        timescales=(3,),
        baselines=("LR_FPR", "LR_Platt_FPR"),
        n_bins=10,
):
    rows = []

    for t in timescales:
        if t not in baseline_results:
            continue

        for name in baselines:
            if name not in baseline_results[t]:
                continue

            r = baseline_results[t][name]
...
def export_priority1_baseline_table(
        baseline_results: dict,
        output_csv: str,
        operating_points: list[tuple[int, float]] = None,
):
    """Export a compact baseline-comparison table for the revision."""
    if operating_points is None:
        operating_points = [(2, 0.20), (3, 0.20), (5, 0.10)]

    rows = []
    for t_ns, fpr_target in operating_points:
        if t_ns not in baseline_results:
            continue
        by_name = baseline_results[t_ns]
        for baseline_name, r in by_name.items():
            ops = r.get('fold_ops', [])
            if baseline_name in {'LR_FPR', 'LR_Platt_FPR'}:
                ops = [op for op in ops if np.isclose(op.get('fpr_target', np.nan), fpr_target)]
            # fixed-threshold baselines are reported once per t, irrespective of FPR target
            if not ops:
                continue
            tp = sum(int(op['tp']) for op in ops)
            fn = sum(int(op['fn']) for op in ops)
            ...
            achieved_fpr = fp / (fp + tn + 1e-8)
            triage_rate = (tp + fp) / (n_test + 1e-8)
            cost_saved = triage_rate * (20.0 - float(t_ns)) / 20.0
            rows.append({
                'timescale_ns': t_ns,
                'reference_fpr_target': fpr_target,
                'baseline': baseline_name,
                ...
                'cost_saved': cost_saved,
                'n_folds': len(ops),
                'heuristic_column': r.get('heuristic_column', None),
            })
...
def expected_calibration_error(y_true, y_prob, n_bins=10, strategy="uniform"):
    ...
    where bins are over predicted probabilities.
    """
    y_true = np.asarray(y_true).astype(int)
    y_prob = np.asarray(y_prob).astype(float)
    ...
    for i in range(n_bins):
        ...
        acc_k = float(y_true[idx].mean())
        conf_k = float(y_prob[idx].mean())
        ece += (nk / n) * abs(acc_k - conf_k)

    return float(ece)


def plot_reliability_diagram_for_timescale(
        results_dict,
        timescale_ns,
        model_name="Logistic Regression",
        out_png="./outputs/reliability_3ns.png",
        n_bins=10,
):
    """
    Reliability diagram using out-of-fold predictions stored in results_dict[strategy][t][model]:
      - y_true: concatenated held-out labels across folds
      - y_prob: concatenated held-out probabilities across folds
    """
    ...
    ax.plot([0, 1], [0, 1], linestyle="--", linewidth=1.2, color="gray", label="Perfect calibration")
    ...
    for strategy_key, by_time in results_dict.items():
        ...
        # Metrics
        ece = expected_calibration_error(y_true, y_prob, n_bins=n_bins, strategy="uniform")
        ...
        ax.plot(
            ...
            color=colors.get(strategy_key, None),
            label=f"{lab} ({model_name})  ECE={ece:.3f}, Brier={brier:.3f}"
        )

    ax.set_xlabel("Mean predicted instability probability")
    ax.set_ylabel("Empirical instability frequency")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ...
    plt.close()
    log.info(f"✓ Saved reliability diagram: {out_png}")
...
def plot_early_time_separation_compact(
    ...
    palette = {"Stable": "#D9D9D9", "Unstable": "#7F7F7F"}
...
def fold_median_pct(ops, key, decimals=1):
    vals = np.asarray([100.0 * float(op.get(key, np.nan)) for op in ops], dtype=float)
...
def export_same_feature_model_baseline_table(
        results_by_strategy_fpr: dict,
        strategy: str = "protein_logo",
        t_ns: float = 5,
        fpr_target: float = 0.20,
        model_names: tuple = ("Logistic Regression", "Random Forest", "Gradient Boosting"),
        output_csv: str = "./outputs/same_feature_model_baselines.csv",
        output_latex: str | None = None,
        long_ns: float = 20.0,
):
    """Export compact same-feature model robustness table at one operating point.

    This table is intended for the revision as a low-cost robustness check:
    every model uses the same four physics-informed features, the same outer
    grouped evaluation split, and the same train-side FPR-threshold calibration
    rule. The comparison is therefore about whether nonlinear tabular models
    improve the FPR-audited deployment metrics, not about changing the policy.

    Default setting: novel-protein leave-one-protein-out, 5 ns, nominal FPR 20%.
    """
    ...
    def _fmt_auc(mean: float, std: float) -> str:
    ...
    any_fpr_key = next(iter(results_by_strategy_fpr.keys()))
    if strategy not in results_by_strategy_fpr[any_fpr_key]:
        raise ValueError(f"Strategy {strategy!r} not found in results_by_strategy_fpr")

    by_time = results_by_strategy_fpr[any_fpr_key][strategy]
    if t_ns not in by_time:
        matches = [k for k in by_time.keys() if isinstance(k, (int, float, np.integer, np.floating)) and np.isclose(float(k), float(t_ns))]
        if not matches:
            raise ValueError(f"Timescale {t_ns} not found for strategy {strategy}")
        t_key = matches[0]
    else:
        t_key = t_ns
    ...
    for model_name in model_names:
        if model_name not in by_time[t_key]:
            log.warning(f"Model {model_name!r} not found at t={t_ns}; skipping")
            continue

        r = by_time[t_key][model_name]
        fold_ops = r.get("fold_ops", []) or []
        ops = [
            op for op in fold_ops
            if np.isclose(float(op.get("fpr_target", -1)), float(fpr_target))
               and int(float(op.get("time_ns", -1))) == int(float(t_ns))
               and op.get("model") == model_name
        ]
        if not ops:
            log.warning(f"No fold_ops for model={model_name}, t={t_ns}, alpha={fpr_target}")
            continue
        ...
        total_unstable = tp + fn
        total_stable = fp + tn
        recall = tp / (total_unstable + 1e-8)
        pooled_fpr = fp / (total_stable + 1e-8)
        triage_rate = (tp + fp) / (n_test + 1e-8)
        cost_saved = sum(
            float(op.get("cost_saved", np.nan)) * int(op.get("n_test", 0))
            for op in ops
            if np.isfinite(float(op.get("cost_saved", np.nan)))
        ) / (n_test + 1e-8)

        rows.append({
            "Model": _display_model(model_name),
            "AUROC": _fmt_auc(float(r.get("auc", np.nan)), float(r.get("auc_std", np.nan))),
            "Unstable stopped / total unstable": f"{tp}/{total_unstable}",
            "Stable stopped / total stable": f"{fp}/{total_stable}",
            "Recall (%)": _fmt_pct(recall),
            "Pooled FPR (%)": _fmt_pct(pooled_fpr),
            "Terminated (%)": _fmt_pct(triage_rate),
            "Cost saved (%)": _fmt_pct(cost_saved),
    ...
    if output_latex:
        ...
        with open(output_latex, "w") as f:
            f.write("\\begin{table}[t]\n")
            f.write("\\centering\n")
            f.write("\\caption{Same-feature nonlinear tabular baselines at the primary novel-protein operating point. All models use the same four physics-informed features, leave-one-protein-out evaluation, and training-side FPR-calibrated threshold selection at 5 ns with nominal $\\alpha=0.20$. Counts and rates are pooled across held-out folds.}\n")
            f.write("\\label{tab:same_feature_model_baselines}\n")
            f.write("\\small\n")
            f.write("\\begin{tabular}{lccccc}\n")
            f.write("\\toprule\n")
            f.write("\\textbf{Model} & \\textbf{AUROC} & \\textbf{Unstable stopped} & \\textbf{Stable stopped} & \\textbf{Pooled FPR} & \\textbf{Cost saved} \\\\\n")
            f.write("\\midrule\n")
            for _, row in df.iterrows():
                f.write(
                    f"{_esc(row['Model'])} & {row['AUROC']} & "
                    f"{row['Unstable stopped / total unstable']} & "
                    f"{row['Stable stopped / total stable']} & "
                    f"{row['Pooled FPR (%)']}\\% & "
                    f"{row['Cost saved (%)']}\\% \\\\\n"
                )
            f.write("\\bottomrule\n")
            ...
            f.write("\\begin{minipage}{0.95\\columnwidth}\n")
            f.write("\\footnotesize\n")
            f.write("\\emph{Note.} AUROC is summarized across leave-one-protein-out folds. The FPR-calibrated decision threshold is selected on the training side of each fold and applied unchanged to the held-out protein.\n")
            f.write("\\end{minipage}\n")
            f.write("\\end{table}\n")

    return df

def export_operating_point_count_table(
        results_by_strategy_fpr: dict,
        frozen_model_by_strategy: dict,
        operating_points_by_strategy: dict,
        output_csv: str,
        output_latex: str | None = None,
):
    """Export count-based audit table for selected operating points.

    For each operating point, reports:
      - unstable complexes correctly triaged / total unstable complexes
      - stable complexes incorrectly triaged / total stable complexes
      - number of folds with undefined FPR because the held-out fold has zero stable negatives

    This table complements median [Q1, Q3] operating summaries by making
    the small-sample counts explicit.
    """

    def _display_regime(strategy: str) -> str:
        if strategy == "protein_logo":
            return "Novel proteins"
        if strategy == "pocket_gkf":
            return "Known protein, new pocket"
        return strategy

    def _display_op(t_ns: float, fpr_target: float) -> str:
        return f"{int(t_ns)} ns @ FPR $\\le$ {int(round(100 * fpr_target))}\\%"

    rows = []

    any_fpr_key = next(iter(results_by_strategy_fpr.keys()))

    for strategy, operating_points in operating_points_by_strategy.items():
        if strategy not in results_by_strategy_fpr[any_fpr_key]:
            continue
        ...
        for t_ns, fpr_target in operating_points:
            if t_ns not in by_time:
                continue
            if model_name not in by_time[t_ns]:
                continue
            ...
            ops = [
                op for op in fold_ops
                if int(op.get("time_ns", -1)) == int(t_ns)
                   and np.isclose(float(op.get("fpr_target", -1)), float(fpr_target))
                   and op.get("model") == model_name
            ]
            ...
            # FPR is undefined in folds with zero stable negatives.
            undefined_fpr_folds = sum(
                1 for op in ops
                if int(op.get("fp", 0)) + int(op.get("tn", 0)) == 0
            )

            rows.append({
                "regime": _display_regime(strategy),
                "operating_point": _display_op(t_ns, fpr_target),
                "model": model_name,
                "unstable_correct": f"{tp}/{total_unstable}",
                "stable_wrong": f"{fp}/{total_stable}",
                "undefined_fpr_folds": f"{undefined_fpr_folds}/{n_folds}",
                "tp": tp,
                "fn": fn,
                "fp": fp,
                "tn": tn,
                ...
                "n_folds": n_folds,
                "n_undefined_fpr_folds": undefined_fpr_folds,
            })
    ...
    if output_latex:
        ...
        with open(output_latex, "w") as f:
            f.write("\\begin{table*}[t]\n")
            f.write("\\centering\n")
            f.write("\\caption{Count-based audit table for selected FPR-calibrated operating points. Counts are pooled across held-out test cases for each evaluation regime.}\n")
            f.write("\\label{tab:count_audit_operating_points}\n")
            f.write("\\small\n")
            f.write("\\begin{tabular}{llccc}\n")
            f.write("\\toprule\n")
            f.write("\\textbf{Evaluation regime} & \\textbf{Operating point} & "
                    "\\textbf{Unstable correctly triaged} & "
                    "\\textbf{Stable incorrectly triaged} & "
                    "\\textbf{Undefined FPR folds} \\\\\n")
            f.write("\\midrule\n")
            ...
            for _, r in df.iterrows():
                ...
                f.write(
                    f"{_esc(regime)} & {_esc(r['operating_point'])} & "
                    f"{r['unstable_correct']} & {r['stable_wrong']} & "
                    f"{r['undefined_fpr_folds']} \\\\\n"
                )
            ...
            f.write("\\footnotesize\n")
            f.write("\\emph{Note.} ``Unstable correctly triaged'' corresponds to true positives among endpoint-unstable complexes. "
                    "``Stable incorrectly triaged'' corresponds to false positives among endpoint-stable complexes. "
                    "FPR is undefined for held-out folds containing no stable complexes.\n")
            f.write("\\end{minipage}\n")
            f.write("\\end{table*}\n")
...
def main():
    ...
    parser.add_argument(
        "--run_priority1_baselines",
        default=True,
        help="Run the revision-critical baseline comparison: LR-FPR vs LR-Platt-FPR vs LR-0.5 vs LR-Youden vs RMSD heuristic."
    )
    parser.add_argument(
        "--baseline_timescales",
        type=float,
        nargs="+",
        default=[2, 3, 5],
        help="Timescales for the priority-1 baseline comparison (default: 2 3 5)."
    )
    parser.add_argument(
        "--heuristic_column",
        type=str,
        default='mean',
        help="Optional explicit RMSD-magnitude column for the heuristic baseline. If omitted, the script will auto-resolve a likely RMSD column."
    )

    parser.add_argument(
        "--run_sequential_baseline",
        default=True,
        help="Run the jointly calibrated sequential/adaptive LR-FPR baseline after priority-1 baselines."
    )
    parser.add_argument(
        "--sequential_horizons",
        type=float,
        nargs="+",
        default=[2, 3, 5],
        help="Horizons for the sequential baseline, evaluated in order (default: 2 3 5)."
    )
    parser.add_argument(
        "--sequential_grid",
        type=int,
        default=41,
        help="Maximum threshold grid size per horizon for joint sequential calibration (default: 41)."
    )
    parser.add_argument(
        "--sequential_objective",
        type=str,
        default="cost_then_recall",
        choices=["cost_then_recall", "recall_then_cost"],
        help="Joint calibration objective for the sequential baseline."
    )
    ...
    if args.run_priority1_baselines:
        log.info("="*80)
        log.info("PRIORITY-1 REVISION BASELINES")
        log.info("="*80)
        baseline_results = evaluate_priority1_decision_baselines(
            features_path=args.features,
            timescales=args.baseline_timescales,
            group_by='protein',
            cv_mode='logo',
            feature_cols=feature_cols,
            fpr_targets=(0.10, 0.20),
        ...
        cal_df = export_baseline_calibration_metrics(
            baseline_results,
            f"./outputs/priority1_baseline_{date_tag}_{version}_calibration.csv",
            timescales=(3, 5),
            baselines=("LR_FPR", "LR_Platt_FPR"),
            n_bins=10,
        )
        print(cal_df.to_string(index=False))
        out_csv = f"./outputs/priority1_baselines_{date_tag}_{version}.csv"
        df_baseline = export_priority1_baseline_table(
            baseline_results=baseline_results,
            output_csv=out_csv,
            operating_points=[(2, 0.20), (3, 0.20), (5, 0.10)],
        )
        log.info(f"✓ Saved priority-1 baseline table: {out_csv}")
        print(df_baseline.to_string(index=False))

        if args.run_sequential_baseline:
            log.info("="*80)
            log.info("SEQUENTIAL / ADAPTIVE LR-FPR BASELINE")
            log.info("="*80)
            seq_results = evaluate_sequential_lr_fpr_baseline(
                features_path=args.features,
                horizons=tuple(args.sequential_horizons),
                group_by='protein',
                cv_mode='logo',
                feature_cols=feature_cols,
                fpr_targets=(0.10, 0.20),
                long_ns=20.0,
                lr_c=3.0,
                max_threshold_grid=args.sequential_grid,
                objective=args.sequential_objective,
            )

            seq_csv = f"./outputs/sequential_lr_fpr_{date_tag}_{version}.csv"
            seq_tex = f"./outputs/sequential_lr_fpr_{date_tag}_{version}.tex"
            df_seq = export_sequential_baseline_table(
                seq_results=seq_results,
                output_csv=seq_csv,
                output_latex=seq_tex,
            )
            log.info(f"✓ Saved sequential baseline table: {seq_csv}")
            log.info(f"✓ Saved sequential baseline LaTeX: {seq_tex}")
            print(df_seq.to_string(index=False))

            compare_csv = f"./outputs/fixed_vs_sequential_lr_fpr_{date_tag}_{version}.csv"
            compare_tex = f"./outputs/fixed_vs_sequential_lr_fpr_{date_tag}_{version}.tex"
            df_compare = export_fixed_vs_sequential_table(
                fixed_results=baseline_results,
                seq_results=seq_results,
                output_csv=compare_csv,
                output_latex=compare_tex,
                fixed_operating_points=((5, 0.20), (5, 0.10)),
            )
            log.info(f"✓ Saved fixed-vs-sequential table: {compare_csv}")
            log.info(f"✓ Saved fixed-vs-sequential LaTeX: {compare_tex}")
    ...
    plot_reliability_diagram_for_timescale(
        results_dict=results_dict,
        timescale_ns=3,
        model_name="Logistic Regression",
        out_png=f"./outputs/figure_reliability_3ns_{date_tag}_{version}.png",
        n_bins=10,
    )
    ...
    # ============================================================================
    # Same-feature nonlinear model robustness table (LR vs RF vs Gradient Boosting)
    # ============================================================================
    model_baseline_df = export_same_feature_model_baseline_table(
        results_by_strategy_fpr=results_by_strategy_fpr,
        strategy="protein_logo",
        t_ns=5,
        fpr_target=0.20,
        model_names=("Logistic Regression", "Random Forest", "Gradient Boosting"),
        output_csv=f"./outputs/same_feature_model_baselines_5ns_fpr20_{date_tag}_{version}.csv",
        output_latex=f"./outputs/same_feature_model_baselines_5ns_fpr20_{date_tag}_{version}.tex",
        long_ns=20.0,
    )
    log.info(f"✓ Saved same-feature model baseline table: ./outputs/same_feature_model_baselines_5ns_fpr20_{date_tag}_{version}.csv")
    log.info(f"✓ Saved same-feature model baseline LaTeX: ./outputs/same_feature_model_baselines_5ns_fpr20_{date_tag}_{version}.tex")
    print(model_baseline_df.to_string(index=False))

    count_df = export_operating_point_count_table(
        results_by_strategy_fpr=results_by_strategy_fpr,
        frozen_model_by_strategy=frozen_model_by_strategy,
        operating_points_by_strategy={
            "protein_logo": [
                (5, 0.20),
                (3, 0.10),
                (5, 0.10),
        ...
        output_csv=f"./outputs/count_audit_operating_points_{date_tag}_{version}.csv",
        output_latex=f"./outputs/count_audit_operating_points_{date_tag}_{version}.tex",
    )

    log.info(f"✓ Saved count audit table: ./outputs/count_audit_operating_points_{date_tag}_{version}.csv")
    log.info(f"✓ Saved count audit LaTeX: ./outputs/count_audit_operating_points_{date_tag}_{version}.tex")
